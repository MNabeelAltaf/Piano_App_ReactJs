...
const Piano = (props) => {
    ...
    useEffect(() => {
        if (right_keys.length !== 0) {
            eliminating_correct_keys();
        }
    }, [right_keys]);
    ...
    const eliminating_correct_keys = () => {

        if (keys.length === 0) {
            return;
        }

        const firstIndex = piano_keys.indexOf(right_keys[0]);
        const lastIndex = piano_keys.lastIndexOf(right_keys[right_keys.length - 1]);

        if (firstIndex !== -1) {
            const matchingKeys = piano_keys.slice(firstIndex, lastIndex + 1);

            const matchingObject = Object.fromEntries(matchingKeys.map((key, index) => [firstIndex + index, key]));

            const matchedKeys = right_keys
                .flatMap(key => Object.entries(matchingObject).filter(([index, value]) => value === key).map(([index]) => +index));

            matchedKeys.forEach(id => {
                const piano_note = document.getElementById(id);
                if (piano_note) {
            ...
            <div className="wrapper">
                ...
                <ul className="piano-keys">
                    ...
                    <li className="key black" data-key="t2" onClick={() => playTune("gb2")}>
                        ...
                        <p id="26" style={{ color: "white", textAlign: 'center' }} className={keys.find(key => key === 'C#' || key === 'Db') ? `${eliminating_correct_keys()} ` : ''}>
                            {piano_keys[26]}
                        </p>
                    </li>
